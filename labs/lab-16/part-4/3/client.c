...
int main(void)
{
    ...
    struct iphdr *ip_header = (struct iphdr *)(packet + sizeof(*eth_header));
    struct udphdr *udp_header = (struct udphdr *)((char *)ip_header + sizeof(*ip_header));
    const char text[] = "raw ethernet header";
    size_t data_len = strlen(text);
    size_t ip_len = sizeof(*ip_header) + sizeof(*udp_header) + data_len;
    ...
    if (setsockopt(send_fd, IPPROTO_IP, IP_HDRINCL, &opt, sizeof(opt)) == -1) {
        perror("setsockopt");
        close(send_fd);
        return 1;
    }
    ...
    if (sendto(send_fd, ip_header, ip_len, 0,
